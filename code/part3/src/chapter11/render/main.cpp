		...
		while (file.getline(line, sizeof(line)))
		{
			...
			else if (strcmp("f", line_head) == 0)
			{
				...
				for (int i = 0; i < 3; i++)
				{
					...
					vbo_1.push_back(-Positions[(size_t)index[0] - 1].z);//因为3ds max导出的坐标是右手坐标系，而我们使用的是左手坐标系，z值是相反的
					...
					vbo_2.push_back(-Normals[(size_t)index[2] - 1].z);//因为3ds max导出的坐标是右手坐标系，而我们使用的是左手坐标系，z值是相反的
					vboCount++;
				}
			}
		}
