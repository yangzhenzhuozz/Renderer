...
public:
	...
	U32 texture2D(double u, double v)
	{
		u = u - floor(u);
		v = v - floor(v);
...
void FS(std::vector<double> &varying, U32 &gl_Color)
{
	...
	U32 c = texture->texture2D(varying[0], varying[1]); //从纹理中取得纹素
...
void msg_loop()//消息循环
{
	...
	auto rotate_1 = Matrix::RotateX(angle);
	auto rotate_2 = Matrix::RotateZ(angle);
	...
	gl->clear_color(0xffffffff); //将所有像素值设置为黑色
	gl->draw();					   //渲染画面
	gl->flush();
	end = clock();
...
int main()
{
	...
	gd = new GraphicsDevice(640, 480);
	gl = new GraphicsLibrary(*gd);
...
//让js代码获取显存
extern "C" BYTE *EMSCRIPTEN_KEEPALIVE get_img_buf()
{
	return (BYTE *)gd->g_pBuf;
}