...
#define _FAR 5//far边界
#define _FrontFace_N 0 //normal 不剔除
#define _FrontFace_CW 1 //cw clockwise 保留顺时针
#define _FrontFace_CCW 2 //ccw counterclockwise 保留逆时针
class Point4
{
public:
	double X, Y, Z, W;
...
public:
	...
	void (*FragmentShader)(std::vector<double>& varying, U32& gl_Color) = nullptr;//指向FS的指针
	int FrontFace = _FrontFace_N;//默认不剔除
	void clear_color(U32 c);//将屏幕所有像素的值设置为c
	void clear_depth(double d);//将深度缓冲区的所有深度值设置为d
	void flush();//将后缓冲区的数据刷新到前缓冲区
private:
