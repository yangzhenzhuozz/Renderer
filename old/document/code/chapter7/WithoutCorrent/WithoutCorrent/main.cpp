...
void FillTriangle(Graphics& gp, Point2* Triangle, double* attribute)
{
	Point2 ps[3];
	for (int i = 0; i < 3; i++)
	{
			...
			else
			{
				...
				else//当前取的是一对交点点中的后一个，已经配成对了
				{
					...
					for (int x = (int)edgeStar->X; x < edgeEnd->X; x++)//绘制这对交点组成的线段
					{
						...
						double u, v;//纹理的uv
						u = WeightA * attribute[0] + WeightB * attribute[2] + WeightC * attribute[4];//使用线性插值计算当前绘制像素的u值
						v = WeightA * attribute[1] + WeightB * attribute[3] + WeightC * attribute[5];//使用线性插值计算当前绘制像素的v值
						double px, py;//图片的x,y
...
int main()
{
	...
	Point3 ps1[] = { {-640,-480,100},{-640,480,100},{640,-480,100} };//第一个三角形
	Point3 ps2[] = { {640,480,100},{640,-480,100},{-640,480,100} };//第二个三角形
	Point4 result1[3];
	Point4 result2[3];
